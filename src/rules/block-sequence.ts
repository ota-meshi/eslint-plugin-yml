            ...
            "YAMLSequence:exit"(node) {
                ...
                if (node.style === "flow") {
                    ...
                    context.report({
                        ...
                        messageId: "required",
                        fix:
                            (canFix && buildFixFlowToBlock(node, context)) ||
                            null,
                    })
                } else if (node.style === "block") {
                    ...
                    const canFix =
                        canFixToFlow(sequenceInfo, node, context) &&
                        !hasTabIndent(context)
                    context.report({
                        loc: node.loc,
                        messageId: "disallow",
                        fix:
                            (canFix && buildFixBlockToFlow(node, context)) ||
                            null,
                    })
                }
            },
        ...
        if (metaIndent != null) {
            for (
                ...
                if (
                    compareIndent(
                        metaIndent,
                        getActualIndentFromLine(line, context),
                    ) > 0
                ) {
