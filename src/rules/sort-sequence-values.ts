...
function buildValidatorFromType(
    ...
    let compareValue: Compare</* eslint-disable-next-line @typescript-eslint/no-explicit-any -- ignore */
    any> = ([a, b]) => a <= b
...
export default createRule("sort-sequence-values", {
    ...
    create(context) {
        ...
        return {
            ...
            "YAMLSequence > *:exit"(
                node: YAMLEntry & { parent: AST.YAMLSequence },
            ) {
                if (!node.parent.entries.includes(node)) {
                    return
                }
            ...
            "YAMLSequence:exit"(node: AST.YAMLSequence) {
                const data = new YAMLSequenceData(
                    node,
                    sourceCode,
                    anchorAliasMap,
                )
        ...
        function getBlockEntryStartOffset(data: YAMLEntryData) {
            ...
            if (!beforeHyphenToken) {
                const comment = sourceCode.getTokenBefore(
                    data.aroundTokens.before,
                    {
